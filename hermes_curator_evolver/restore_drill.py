...
def _read_drill_state_with_error(state_path: str | Path) -> tuple[dict[str, Any], str | None]:
    ...
    # traceback (roadmap U94; same containment class as backfill P8).
...
def _restore_support_files(
    *,
    ...
    ``unsafe-backup-path`` (U94 parity — a tampered manifest must not name
...
def run_restore_drill(
    ...
    *,
    ...
    The manifest is untrusted input (roadmap U94 — parity with the
    ...
    if not backup_path_raw:
        ...
        # U94 parity: same containment and refusal string the rollback path
